...
Player::Player( int id ) {
  ...
  this->id = id;
  position = 0.5;
  leftCoordinate = Coordinate( 1.05 * cos( ( 4 - id * 2 ) * TAU / 6 ), 1.05 * sin( ( 4 - id * 2 ) * TAU / 6 ) );
  rightCoordinate = Coordinate( 1.05 * cos( ( 5 - id * 2 ) * TAU / 6 ), 1.05 * sin( ( 5 - id * 2 ) * TAU / 6 ) );
  leftButton = id * 2;
  rightButton = id * 2 + 1;
  switch(id) {
    case 0:
...
void Player::render( Buffer* buffer ) {
  Circle circle( getCoordinate(), 0.14 );
...
void logic_thread() {
  ...
  for( int i = 0; i < 6; i++ ) {
    borders[ i ] = Line( 
      Coordinate( 1.025 * cos( i * TAU / 6 ), 1.025 * sin( i * TAU / 6 ) ), 
      Vector( ( i + 2 ) * TAU / 6, 1 ) 
    );
  }
  ...
  while( 1 ) {
    ...
    while ( 
      ...
      for( int i = 0; i < 6; i++ ) {
        ...
        if( players[ player ].alive ) {
          ...
          if( position.distance( ball.coordinate ) < ballwidth ) {
            ...
            ball.coordinate.x = position.x + ballwidth * cos( circle_norm_angle );
            ball.coordinate.y = position.y + ballwidth * sin( circle_norm_angle );
      ...
      // increase ball speed
      ball.vector.norm = 1.001 * ball.vector.norm;
    ...
    for( float i = 0.0; i < 1.0; i += 0.01 ) {
      Circle circle( players[ winner ].getCoordinate(), i );
      circle.render( buffer, players[ winner ].color );
      buffer->render();
      usleep(10000); // 50 per second
    }
